...
bool GliderManipulator::calcMovement()
{
        ...
        buttonMask==(GUIEventAdapter::LEFT_MOUSE_BUTTON|GUIEventAdapter::RIGHT_MOUSE_BUTTON))
    {
		...
		float pitch = inDegrees(dy*75.0f*dt)*50.0;
		float roll = -inDegrees(dx*50.0f*dt)*50.0;
		...
		/// yaw instead of roll
		roll_rotate.makeRotate(roll,up.x(),up.y(),up.z());
		...
		if (_yawMode==YAW_AUTOMATICALLY_WHEN_BANKED)
		{
			...
			float yaw = 0.2*inRadians(bank)*dt;
