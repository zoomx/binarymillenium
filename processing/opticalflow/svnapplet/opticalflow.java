import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class opticalflow extends PApplet {/**
* optical flow 
*
* binarymillenium
* June 2008
...
public float weight(int i,int j) {
      
        float div = (flow_point.s/2);
        float x = (i - div)/div;
        float y = (j - div)/div;
        
        return 1.0f-(sqrt(x*x + y*y)/sqrt(2.0f));
...
class flow_point {
    ...
    flow_point(float x, float y) {
      ...
      pixel_block.pixels = new int[s*s];
      
      old_pixel_block = new PImage();
      old_pixel_block.width = s;
      old_pixel_block.height = s;
      old_pixel_block.pixels = new int[s*s];
      
      diff_x = new PImage();
      diff_x.width = s;
      diff_x.height = s;
      diff_x.pixels = new int[s*s];
      
      diff_y = new PImage();
      diff_y.width = s;
      diff_y.height = s;
      diff_y.pixels = new int[s*s];
      
      diff_t = new PImage();
      diff_t.width = s;
      diff_t.height = s;
      diff_t.pixels = new int[s*s];
...
public void setup() {
  ...
  old_pixels = new int[width*height];
  ...
  //loadPixels();
    ...
    final float div = 42.0f;
    final float div2 = 25.0f;
     final float div3 = 115.0f;
    ...
    a = new PImage();
     ...
     for (int i = 0; i < a.width; i++) {
       for (int j = 0; j < a.height; j++) {
         ...
         float val = (0.4f*noise(i/div,j/div) + 
                            0.4f*noise(i/div2,j/div2) + 
                            0.4f*cos(PI*noise(i/div3,j/div3)) + 
                            0.4f*cos(PI*noise(i/150.0f,j/100.0f)) +
                            0.4f*cos(PI*noise(i/25.0f,j/55.0f)));
...
public float[][] i_xy(int[][] pixel_block) {
  ...
  for (int j = 1; j < pixel_block[i].length-1; j++) {
   ...
   /// the weight of the filter is 8, so divide it out
    sobel_x = (b00 + 2*b01 + b02 - b20 - 2*b21 - b22)/8.0f;  
    sobel_y = (b00 + 2*b10 + b20 - b02 - 2*b12 - b22)/8.0f;                     
...
public float[] i_t(int[][] pixel_block,int[][] old_pixel_block) {
...
/// return Vx Vy of 
public float[] optical_flow(flow_point tfp) {
...
public void process_flow_points()
{
   for (int i = 0; i < fp.length; i++) {
...
has_initted = true;
  ...
  static public void main(String args[]) {     PApplet.main(new String[] { "opticalflow" });  }}