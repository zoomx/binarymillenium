...
void setup() {
  ...
  /// TBD - this needs to be sent from server 
  reader = createReader("../../depthbuffer/angles.csv");
  cur_r = PI/9;

  perspective(angle, float(width)/float(height),near,far);
  //frameRate(1);
  println("client: " + near + " " + far + ", " + wonegrid + " " + honegrid + ", gridscale " + gridscale);
...
void loadgrid() {
  BufferedReader gridreader = createReader("../depthdemoserver/grid_final.csv");
  ...
  println("client: finished loading grid " + count);
}
