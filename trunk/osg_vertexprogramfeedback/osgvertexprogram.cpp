...
float fresnel = 0.4;           // Fresnel multiplier
...
osg::TextureCubeMap* readCubeMap(osg::Image* image)
{
...
*/
    ...
    if (image) //PosX && imageNegX && imagePosY && imageNegY && imagePosZ && imageNegZ)
    {
        ...
        cubemap->setWrap(osg::Texture::WRAP_S, osg::Texture::MIRROR);
        cubemap->setWrap(osg::Texture::WRAP_T, osg::Texture::MIRROR);
        cubemap->setWrap(osg::Texture::WRAP_R, osg::Texture::MIRROR);
...
public:
    ...
    virtual void operator()(osg::Node* node, osg::NodeVisitor* nv)
    {
        ...
        if (cv)
        {
		/// make the cube map rotate with the scene
...
osg::Node* createSkyBox(osg::Image* image)
{
    ...
    osg::TextureCubeMap* skymap = readCubeMap(image);
...
osg::Node* addRefractStateSet(osg::Node* node,osg::Image* image)
{
    osg::StateSet* stateset = new osg::StateSet();
...
int main(int argc, char *argv[])
{
    ...
    rootnode->addChild(createSkyBox(image));
    ...
    rootnode->addChild( addRefractStateSet(model,image) );
...
bool saveimages = false;
    ...
    while( !viewer.done() )
    {
        ...
        for (int i = 0; i < 512*512; i++) {
            sum += data[i];
            data[i] = (unsigned char)(data[i]*autogain);
        }
            avg = sum/(512.0*512.0*256.0) ;
                if (avg < target) {
                autogain += (target-avg)*0.06;
                if (autogain > 4.0) autogain = 4.0;
            } else {
                autogain -= (avg - target)*0.06;
                if (autogain < 0.0) autogain = 0.0;
            }
               
               //std::cout << "avg " << avg << ", gain " << autogain << std::endl;
#endif
        }
