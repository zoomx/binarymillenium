...
void getNewStillImage(char* filename);
...
int main(int argc, char *argv[])
{
	...
	getNewStillImage("images/smallnow1.jpg");
...
void getNewStillImage(char* filename) {
	...
	gXsize = image->columns;
	gYsize = image->rows;
	
	printf("%s %d, gXsize %d, gYsize %d  \n"  ,filename, gPatt.loop_num, gXsize,gYsize);

	gARTImage = malloc(sizeof(ARUint8) * 3 * image->rows * gXsize);
	int y;
	index = 0;
	for (y=0; y < (long) image->rows; y++)
	{
		const PixelPacket *p = AcquireImagePixels(image,0,y,gXsize,1,&image->exception);
		if (p == (const PixelPacket *) NULL)
			break;
		int x;
		for (x=0; x < (long) gXsize; x++)
		{
			/// convert to ARUint8 gARTImage
			/// probably a faster way to give the data straight over
			/// in BGR format
			gARTImage[index*3+2]   = p->red;
			gARTImage[index*3+1] = p->green;
			gARTImage[index*3] = p->blue;
			p++;
			index++;
		}
	}
	...
	if (button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN) {
		...
		if (state == GLUT_DOWN) {
			if (gStatus == 0 && gPatt.loop_num < LOOP_MAX) {
				...
				sprintf(filename, "images/smallnow%d.jpg", (gPatt.loop_num+1));
				getNewStillImage(filename);
