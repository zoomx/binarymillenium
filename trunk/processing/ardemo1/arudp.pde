...
public class arUdp {
  ...
  if (have_info) {
    ...
    int cnt = 0;
    //println (markerInfos.length);
    for (int i = 0; i < markerInfos.length; i++) {
     
      if (markerInfos[i].cf > 0.6) {// && markerInfos[i].id >= 0 ) { 
        ...
        if ((newId >= 0) && (newId < numMarkers)) {
          ...
          dbInfos[newId].oldy = dbInfos[newId].y;  
           dbInfos[newId].oldrot = dbInfos[newId].rot;
            ...
            dbInfos[newId].oldrot = markerInfos[i].rot+PI;
          }
          
          dbInfos[newId].x = markerInfos[i].x; 
          dbInfos[newId].y = markerInfos[i].y;
          dbInfos[newId].rot = markerInfos[i].rot+PI; 
  ...
  public float findBestCf(int id) {
    
    float bestcf = 0.0;
     for (int i = 0; i < markerInfos.length; i++) {
       
        if (markerInfos[i].id == id )  {
           if (markerInfos[i].cf > bestcf) bestcf =  markerInfos[i].cf;
        }
     }
